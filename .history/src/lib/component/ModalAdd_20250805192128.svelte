 <script>
  ...
  function pesanProducts() {
    ...
    if (!selectedSize) messages.push("Silakan pilih ukuran");
    if (!selectedColor) messages.push("Silakan pilih warna");

    if (messages.length > 0) {
      warnAlert = messages.join(" ");
      setTimeout(() => {
        warnAlert = "";
      }, 5000);
      return; // Stop proses jika belum lengkap
    }
          ...
          <div class="font-syerif">
            ...
            <div class="w-full flex flex-col border-y border-dotted py-2">
              ...
              <form class="filter">
                ...
                <input class={`btn bg-${col} text-transparent`} type="radio" name="color" aria-label={col} value={col}
